...
def train_test(model, epochs = 1000, batch_size = 128, show_after = 100):
    train_losses = []; test_losses = []
    train_acc = [];    test_acc = []
    
    for e in tqdm(range(1,epochs+1)):
        x, y = get_batch(k = model.k, batch_size = batch_size, test = False)
        predicted = model(x)
        ...
        with torch.no_grad():
            x, y = get_batch(k = model.k, test = True)
            predicted = model(x)
            loss = F.nll_loss(predicted, y.to(device))
